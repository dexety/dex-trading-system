...
with open(f"{path_to_output}/indicators_{date_borders}.csv", "w") as output:
    filenames = [f"{path_to_output}/parts_{date_borders}/{i}.csv" for i in range(int(sys.argv[1]))]
    concatenated_csv = pd.concat(
        [pd.read_csv(filename) for filename in filenames]
    )
    concatenated_csv.to_csv(f"{path_to_output}/indicators_{date_borders}.csv", index=False) 
